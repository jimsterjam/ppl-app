...
function render() {
  ...
  filtered.forEach((e, i) => {
    ...
    const blocks = [];
    if (e.originalFeedbackText) {
      const origDetails = (e.originalViolationDetails || []).map((v) => \`Regel \${v.rule}: \${v.issue || ''}\${v.quote ? \` ("\${v.quote}")\` : ''}\`).join('\\n');
      blocks.push(\`<div class="textlabel">Original-Feedback\${(e.originalTriggeredRules||[]).length ? ' - Regel(n) ' + e.originalTriggeredRules.join(', ') : ''}</div><div class="textblock">\${e.originalFeedbackText.replace(/</g, '&lt;')}</div>\${origDetails ? \`<div class="textlabel">Konkrete Begründung</div><div class="textblock">\${origDetails.replace(/</g, '&lt;')}</div>\` : ''}\`);
    }
    ...
    if (e.revisionAttempted && e.revisionSucceeded === false) {
      const recheckDetails = (e.revisionRecheckDetails || []).map((v) => \`Regel \${v.rule}: \${v.issue || ''}\${v.quote ? \` ("\${v.quote}")\` : ''}\`).join('\\n');
      const recheck = recheckDetails || 'keine (Re-Prüfung selbst ist fehlgeschlagen/uneindeutig, siehe Konsolen-Log dieses Laufs)';
